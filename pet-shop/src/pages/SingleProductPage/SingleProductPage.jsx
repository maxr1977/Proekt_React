...
import { fetchSingleProduct, clearCurrentProduct } from '../../store/slices/productsSlice';
...
export default function SingleProductPage() {
    ...
    if (status === 'loading' || !product) {
        return <div className={styles.status_message}><p>Loading...</p></div>;
    }
    if (error) {
        return <div className={styles.status_message}><p>Error: {error}</p></div>;
    }
    ...
    const imageGallery = [product.image, product.image, product.image].map(img => `${import.meta.env.VITE_API_BASE_URL}${img}`);

    const breadcrumbPaths = [
        { label: 'Main Page', path: '/' },
        { label: 'Products', path: '/products' },
        ...(productCategory ? [{ label: productCategory.title, path: `/categories/${productCategory.id}` }] : []),
                ...
                <div className={styles.product_content}>
                    ...
                    <div className={styles.gallery_section}>
                        <div className={styles.thumbnail_list}>
                            {imageGallery.map((imgUrl, index) => (
                                <div 
                                    key={index}
                                    className={`${styles.thumbnail_wrapper} ${index === activeImageIndex ? styles.active_thumbnail : ''}`}
                    ...
                    <div className={styles.info_section}>
                        ...
                        <div className={styles.price_wrapper}>
                            <p className={styles.new_price}>${product.discont_price ?? product.price}</p>
                            {product.discont_price && <p className={styles.old_price}>${product.price}</p>}
                            {discount > 0 && <div className={styles.discount_badge}>-{discount}%</div>}
                        </div>
                        ...
                        <div className={styles.actions_wrapper}>
                            ...
                            {isAdded ? (
                                <UIButton theme='added' disabled>Added to cart</UIButton>
                            ) : (
                        ...
                        <div className={styles.description}>
                            ...
                            {product.description.length >= 300 && (
                                <span className={styles.read_more_btn} onClick={() => setIsExpanded(!isExpanded)}>
